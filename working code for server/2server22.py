...
client_pub = base64.b64decode(data_frags)
ct, shared = frodo.encaps(client_pub)
encoded = base64.b64encode(ct)
...
# Create a large response packet of 9000 bytes
# create a TCP data packet with the message to send to the server
server_fragments = fragment_message(encoded, client_port=client_port)
...
# receive the FIN from client
fin_from_client = s.recv()
print("got fin ", fin_from_client.show())
print("sending fin ack")
# Create the FIN ACK packet
fin = IP(dst=client_ip)/TCP(sport=src_port, dport=client_port, flags="FA", ack=fin_from_client.seq + 1, seq = fin_from_client.ack)
s.send(fin)
